...
$_POST['material_mark'] = strip_tags($_POST['material_mark']);
$_POST['service_mark']  = strip_tags($_POST['service_mark']);
$_POST['message']       = strip_tags($_POST['message']);

$message  = '';
$message .= "Оценка материала: {$_POST['material_mark']} <br>\n";
$message .= "Оценка сервиса: {$_POST['service_mark']} <br>\n";
...
if ($result ) {
	...
	if (!$dbConnect->connect_error) {
		...
		$material_mark = (int)$_POST['material_mark'];
		$service_mark = (int)$_POST['service_mark'];
		$text = $_POST['message'];
		$dbConnect->query("SET NAMES 'utf8'");
		$dbConnect->query("INSERT INTO mails (sent_date, mail_from, material_mark, service_mark, text) VALUES ('$mail_date','$from','$material_mark','$service_mark','$text')");
	}
